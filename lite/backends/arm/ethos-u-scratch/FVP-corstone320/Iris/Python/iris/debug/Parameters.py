...
#
# \date   Copyright ARM Limited 2016-2019,2021-2022,2024 All Rights Reserved.
...
class ModelStringParameter(str, ModelParameter):
    ...
    def __new__(cls, info, value = None):
        if value is not None:
            ...
            return str.__new__(cls, info.parameterInfo.get('defaultString'))
...
class ParameterDict(dict):
    ...
    def values(self):
        # real dictionaries return a dictview, here we return an iterator
        for k in iter(self.infos.keys()):
    ...
    def __repr__(self):
        r = "{"
        for k in self.infos.keys(): # grab values not cached already
            r += "{!r}: {!r}, ".format(k, self[k])
        return r[0:-2] + "}"
