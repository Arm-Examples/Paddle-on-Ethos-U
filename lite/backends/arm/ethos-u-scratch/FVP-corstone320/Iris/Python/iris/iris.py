...
# \date   Copyright ARM Limited 2015-2024 All Rights Reserved.
...
if sys.version_info < (3,0):
    ...
    unicode = str
    # long and int are unified (PEP 237)
    long = int
...
class IrisPythonAdapter(object):
    ...
    def instanceRegistry_getList(self, **kwargs):
        ...
        try:
            ...
            if e.code == error.E_unsupported_argument_name:
...
class IrisClientBase(object):
    ...
    messages.
    """
    def __init__(self, instanceName, instance, verbose, msg_format=None, timeoutInMs = 10000):
        """Constructor.
        ...
        # Receive thread signal.
        self.sockTerminateReceiveThreadProcRead = None
        self.sockTerminateReceiveThreadProcWrite = None
        # Python 2.7 on Windows does not have socketpair().
        if hasattr(socket, "socketpair"):
            # Socketpair used to terminate the receiveThreadProc() (by unblocking the select()).
            (self.sockTerminateReceiveThreadProcRead, self.sockTerminateReceiveThreadProcWrite) = socket.socketpair()
    ...
    def log(self, message):
        """Log function used for local debugging.
        """
        print("IrisClient: {}: {}".format(self.instanceName, message))
    ...
    def _connect_epilog(self):
        ...
        # Start receive loop.
        if hasattr(socket, "socketpair"):
            self.receiveThread = threading.Thread(target = self.receiveThreadProc_SP, name = self.instanceName + '.receive_thread')
        else:
            self.receiveThread = threading.Thread(target = self.receiveThreadProc_noSP, name = self.instanceName + '.receive_thread')
    ...
    def disconnect(self, force = False, unregister_exit_handler = True, closeSocket = True):
        ...
        if force:
            ...
            # Clear self.receivedRequests without processing any
            # pending requests.
            while True:
                try:
        ...
        if self.sockTerminateReceiveThreadProcRead != None:
            self.sockTerminateReceiveThreadProcRead.close()
        if self.sockTerminateReceiveThreadProcWrite != None:
            self.sockTerminateReceiveThreadProcWrite.close()
    ...
    def receiveThreadProc_noSP(self):
        """Receive loop.
        ...
        while self.isConnected_:
            ...
            if needResync:
                self.log("error: IrisTcpClient: need to re-sync after receiving trash")
            ...
            while ((len(receiveBuf) >= 20 and receiveBuf.startswith(b'IrisJson:'))
                ...
                if receiveBuf.startswith(b'IrisJson:'):
                    ...
                    if len(parts) != 3:
                        self.log("error: IrisTcpClient: received trash after 'IrisJson:'")
                        needResync = True
                        break
                    ...
                    try:
                        ...
                        self.log("error: IrisTcpClient: parse error in content length integer after 'IrisJson:'")
                        needResync = True
                        break
                    ...
                    if parts[2][contentLength:contentLength+1] != b'\n':
                        self.log("error: IrisTcpClient: missing LF after IrisJson message")
                        needResync = True
                        break
    ...
    def receiveThreadProc_SP(self):
        ...
        while self.isConnected_:
            ...
            try:
                ...
                if self.sockTerminateReceiveThreadProcRead != None:
                    rlist.append(self.sockTerminateReceiveThreadProcRead.fileno())
                xlist = [self.sock.fileno()]
                (rReady, wReady, xReady) = select.select(rlist, [], xlist)
                if (len(rReady) == 0) and (len(xlist) == 0):
                    # Timeout or something weird. Should never happen. Ignore and try again.
                    continue
                if (self.sockTerminateReceiveThreadProcRead != None) and (self.sockTerminateReceiveThreadProcRead.fileno() in rReady):
                    # Got wakeup signal: Stop receive thread?
                    # Clear wakeup signal. (4096: Clear all bytes in case we got notified multiple times.)
                    self.sockTerminateReceiveThreadProcRead.recv(4096)
                    continue # Check self.isConnected_.
                if self.sock.fileno() in rReady:
                    # Will never block.
                    receiveBuf += self.sock.recv(4096)
            except socket.error:
                self.sock.close()
    ...
    def receiveThreadReceivedMessage(self, message):
        ...
        if not isinstance(message, dict):
            self.log("error: IrisTcpClient: receivedThreadReceivedJsonMessage(): message must be an object")
    ...
    def receiveThreadReceivedJsonMessage(self, messageText):
        ...
        """
        if self.verbose:
            message = json.loads(messageText, object_hook=self.jsonIntegerObjectWarning)
        else:
            message = json.loads(messageText)
        self.receiveThreadReceivedMessage(message)

    def jsonIntegerObjectWarning(self, obj):
        if '__Iris64L' in obj:
            self.log("warning: received integer encoded as object; this is not supported by the Iris Python library: {}".format(obj))
        return obj
    ...
    def processRequest(self, request):
        ...
        if not isValidRequest(request):
            self.log('error: IrisTcpClient: Received invalid request {}'.format(request))
            if hasResponse:
                response['error'] = {}
    ...
    def _setIsConnected(self, isConnected):
        """Set connection state and terminate receive thread if !isConnected.
        """
        wasConnected = self.isConnected_
        self.isConnected_ = isConnected
        if wasConnected and (not self.isConnected_) and (self.sockTerminateReceiveThreadProcWrite != None):
            # Wake up select() in receiveThreadProc() loop to make receive thread exit.
            self.sockTerminateReceiveThreadProcWrite.send(b"a")
...
class IrisTcpClient(IrisClientBase):
    ...
    def connect(self, hostname, port, timeoutInMs=None):
        ...
        if self.verbose >= 1:
            self.log("IrisTcpClient.connect(hostname={}, port={}, timeoutInMs={})".format(hostname, port, timeoutInMs))
...
class IrisUnixDomainClient(IrisClientBase):
    ...
    def connect(self, sock, timeoutInMs=None):
        ...
        if self.verbose >= 1:
            self.log("IrisUnixDomainClient.connect(sock={}, timeoutInMs={})".format(sock.fileno(), timeoutInMs))
        if self.isConnected_:
            raise error.IrisError(error.E_already_connected, 'Already connected to a server')
...
def decodeResourceReadResult(resourceInfos, resourceReadResult):
    ...
    for regInfo in resourceInfos:
        ...
        if type in ('numeric', 'numericSigned', 'numericFp'):
            ...
            raise error.IrisError(error.E_internal_error, "Unknown resource type {}".format(regInfo.type))
...
def try_iris(method, default, **kwargs):
    try:
        ...
        if e.code.number not in {error.E_no_response_yet,
