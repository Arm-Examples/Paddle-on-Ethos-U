...
#
# \date   Copyright ARM Limited 2020-2024 All Rights Reserved.
...
class ModelBase(object):
    ...
    def __init__(self, client, verbose, synchronous, socketpairpool=None):
        self.client = client
        self.verbose = verbose
        self.synchronous = synchronous
        self.socketpairpool = socketpairpool
        self._pid = None
    ...
    def _new_target(self, target_info):
        # see SyncModel and AsyncModel for implementations
    ...
    def _shutdown_model(self):
        """Request shutdown without disconnecting the TCP connection.
        """
    ...
    def isFork(self):
        """This model was created by fork().
        """
    ...
    def release(self, shutdown=False):
        ...
        :param shutdown:
            For TCP connections: If True, the simulation will be shutdown and any other scripts or debuggers
            will have to disconnect. If False, a simulation might be kept alive after disconnection.

            For UNIX domain socket connections: This should always be set to False as disconnecting from 
            the model will automatically shut down the model. Setting this to True will work but will 
            result in a non-deterministic shutdown sequence which is undesirable.
        """
        if self.isConnected():
            # If we are a fork, ask model explicitly to shut itself down via simulation_requestShutdown()
            # because fork clients do not close their sockets (which would automatically shut the model down)
            # because sockets are recycled for future forks. 
            if shutdown or self.isFork():
                self._shutdown_model()
            sock = self.client.sock
            self.client.disconnect(closeSocket = not self.isFork())
            if self.isFork():
                self.socketpairpool.releaseSocketpair(sock)

        if self.isFork():
            # If we are a fork, wait until our associated process exited.
            # The most reliable way is to do this via the parent client
            # and parent model, which implies that all child processes must
            # terminate before their parents. Except for this "waitpid()"
            # there is no other parent/child-dependency.
            exitStatus = self._parent_model.forkWait(self._pid)
            self._pid = None
            return exitStatus

    def fork(self, client_name = None):
        """Fork model and return a connected client.

        Use release() on the returned model to shut the model down and wait for the subprocess to exit.

        :param client_name
            Instance name of the returned Python client. This should begin with "client.".
            The default is the client name of the main client plus a suffix indicating the fork tree path using the socket file descriptors.
            Note that when having multiple forks, all client_names can be set to an identical name and the Iris instance name uniquification
            will not make them unique (different) because all client names live in independent model processes.
            Setting client_name to different values for different forks is useful for debugging/logging purposes but is not necessary otherwise.
        """
        if os.name == 'nt':
            raise NotImplementedError()
        lhs, rhs = self.socketpairpool.allocSocketpair()
        connectionSpec = "socketfd={}".format(rhs.fileno())
        try:
            simulationForkResult = self.client.irisCall().simulation_fork(instId=0, connect=connectionSpec)
        except Exception:
            raise
        try:
            if client_name == None:
                client_name = self.client.instanceName + "_f{}".format(lhs.fileno())
            model = NewUnixDomainSocketModel(lhs, timeoutInMs=self.client.timeoutInMs, client_name=client_name, synchronous=self.synchronous, verbose=self.verbose, waitForInstantiation=False, socketpairpool=self.socketpairpool)
            model._pid = simulationForkResult.childPid
            model._parent_model = self
            return model
        except Exception:
            raise

    def forkWait(self, pid):
        """Wait for child process to exit.

        Return exit status of child process.
        """
        simulationForkWaitResult = self.client.irisCall().simulation_forkWait(instId=0, pid=pid)
        return simulationForkWaitResult.exitStatus
...
class AsyncModel(ModelBase):
    ...
    """
    def __init__(self, client, verbose, socketpairpool=None):
        ModelBase.__init__(self, client, verbose, synchronous=False, socketpairpool=socketpairpool)
...
class SyncModel(ModelBase):
    ...
    """
    def __init__(self, client, verbose, socketpairpool=None):
        ModelBase.__init__(self, client, verbose, synchronous=True, socketpairpool=socketpairpool)
...
def _NewModel(synchronous, client, verbose, socketpairpool=None):
    if synchronous:
        return SyncModel(client, verbose, socketpairpool)
    else:
        return AsyncModel(client, verbose, socketpairpool)
...
def NewNetworkModel(host = "localhost", port = 0, timeoutInMs = 20000, client_name = "client.iris_debug", synchronous = False, verbose = False):
...
class NetworkModel(AsyncModel):
    ...
    def __init__(self, host = "localhost", port = 0, timeoutInMs = 20000, client_name = "client.iris_debug", verbose = False):
        """
        Connects to an already initialised Iris server
...
def NewUnixDomainSocketModel(sock, timeoutInMs=20000, client_name="client.iris_debug", synchronous=False, verbose=False, socketpairpool=None, waitForInstantiation=True):
    """
    ...
    return _NewModel(synchronous, client, verbose, socketpairpool)
