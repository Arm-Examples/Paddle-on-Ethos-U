# \brief  U64Json encoder/decoder, provides a similar API to the Python json module
# \date   Copyright ARM Limited 2018,2021-2024 All Rights Reserved.
...
if _has_typecode_Q():
    U64Array = functools.partial(array.array, 'Q')
elif _typecode_L_is_64bit():
    U64Array = functools.partial(array.array, 'L')
else:
    ...
    class U64Array(object):
        ...
        def _get_u8_indices(self, key):
            ...
            if isinstance(key, numbers.Integral):
                ...
                value_is_array = True
                if step != 1:
                    raise KeyError('U64Array does not support strides other than 1')
            else:
...
def _decode(buffer):
    ...
    if sys.byteorder == 'big':
        # U64Json data is always little-endian so it must be byteswapped on big-endian systems.
        buffer = U64Array(buffer) # Avoid modifying the caller's buffer by making a local copy
        assert(buffer.itemsize == 8)
        buffer.byteswap()
