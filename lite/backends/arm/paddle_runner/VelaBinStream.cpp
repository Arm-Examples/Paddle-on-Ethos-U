...
bool vela_bin_validate(const char* data, int size) {
  ...
  if ((uintptr_t)data != next_mul_16((uintptr_t)data)) {
    std::cout << "Vela bin ptr not aligned to 16 bytes: " << (void*)data << std::endl;
    valid = false;
  }
  if ((uintptr_t)foot != next_mul_16((uintptr_t)foot)) {
    std::cout << "End of vela bin not aligned to 16 bytes: " <<  (void*)foot << std::endl;
    valid = false;
  }

  // Check header and footer blocks are the right format
  if (strncmp(data, "vela_bin_stream", strlen("vela_bin_stream")) != 0) {
    std::cout << "Incorrect header in vela_bin_stream" << std::endl;
    valid = false;
  }
  if (strncmp(foot, "vela_end_stream", strlen("vela_end_stream")) != 0) {
    std::cout << "Incorrect footer in vela_bin_stream" << std::endl;
    valid = false;
  }
...
bool vela_bin_read(const char* data, VelaHandles* handles, int size) {
  ...
  while (ptr - data < size) {
    ...
    if (!strncmp(b->name, "vela_bin_stream", strlen("vela_bin_stream"))) {
      ...
      if (ptr - data != size) {
        //std::cout << "Expected vela binary to end with vela_end_stream" << std::endl;
        return false;
      }
      return true;
    } else {
      // Unrecognised block name
      //std::cout << "Invalid block name or malformed binary" << std::endl;
      return false;
    }
  }
