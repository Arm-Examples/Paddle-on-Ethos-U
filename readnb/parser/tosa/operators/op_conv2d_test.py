...
from parser.tosa.tosa_quant_utils import build_rescale_conv_output, build_relu, build_hardswish
...
#            weight_name = weight.name
        ...
        if input.dtype == ts.DType.INT8 and output.dtype == ts.DType.INT8:
            if attr_dict.get("with_act", False):
                ...
                elif attr_dict["act_type"] == "hard_swish":
                    build_hardswish(tosa_graph, rescale_output, output, output_scale[0], output_scale[0])
                else:
                    raise RuntimeError(
                        "Only [Relu, HardSwish] is supported as activation function in fused_conv2d node"
                    )
