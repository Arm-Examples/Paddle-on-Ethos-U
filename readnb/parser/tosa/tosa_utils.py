...
import torch
from .tosa_mapping import TosaArg
...
def getNodeArgs(inputs: list) -> list[TosaArg]:
    print("\n")
    print(f"Node Op input gets {inputs}")
    # return [TosaArg(arg) for arg in node.args]
    return [TosaArg(arg['arguments'][0]) for arg in inputs]


def getNodeOutArgs(outputs: list) -> list[TosaArg]:
    print("\n")
    print(f"Node Op output gets {outputs}")
    # return [TosaArg(arg) for arg in node.args]
    return [TosaArg(out) for arg in outputs for out in arg['arguments']]
