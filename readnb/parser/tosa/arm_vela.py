...
def vela_compile(tosa_graph, args: List[str], shape_order=None, out_dir=None):
    print("doint vela compile")
    import time
    time_back = "_".join(str(time.time()).split("."))

    with tempfile.TemporaryDirectory() as tmpdir:
        # tmpdir = f"/home/wangteng/storage/work_project/arm_etos_paddlelite/paddlelite_arm/arm-paddle/Paddle-Lite-Ethos/models/convert_out/{time_back}"
        if out_dir is None:
            return None
        print(out_dir)
        tmpdir = out_dir
        ...
        print(f"use tosa file {tosa_path}")
        ...
        with np.load(np_path, allow_pickle=False) as data:
            ...
            for key in bin_blocks.keys():
                ...
                blocks = blocks + block
        with open(f"{tmpdir}/out_vela.bin", "wb+") as f:
            f.write(blocks)
        return blocks
